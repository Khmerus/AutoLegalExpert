...
const App: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    if (files.length === 0) {
      setError("Загрузите документы для проверки.");
      return;
    }
    if (!isVerified) {
    ...
    try {
      ...
      setError(err.message || "Ошибка системы аналитики.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans selection:bg-indigo-100">
        ...
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mt-1">Audit System v5.2</p>
            </div>
          </div>
          <div className="flex gap-2 items-center px-3 py-1 bg-slate-100 rounded-full border border-slate-200">
            <div className="w-2 h-2 bg-indigo-500 rounded-full animate-pulse"></div>
            <span className="text-[9px] font-black uppercase text-slate-600 tracking-tighter">Gemini 3 Pro Active</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-16 space-y-20">
        <div className="text-center space-y-6 max-w-2xl mx-auto animate-fade-in">
          <h2 className="text-5xl font-black tracking-tighter text-slate-900 leading-[1.1]">
            Ваш личный эксперт по <br/>
            <span className="text-indigo-600">переоборудованию ТС</span>
          </h2>
          <p className="text-slate-500 text-lg font-medium leading-relaxed">
            Загрузите документы, и наш ИИ проверит их на соответствие всем требованиям ГИБДД за считанные секунды.
          </p>
        </div>
          ...
          <div className="lg:col-span-5 space-y-12">
            <section className="space-y-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">Выберите этап</h3>
              </div>
              <div className="grid gap-3">
                {[
                  { id: AppStage.PRELIMINARY, title: 'Предварительный этап', desc: 'Доверенность + Заключение' },
                  { id: AppStage.FINAL, title: 'Финальный этап', desc: 'Протокол + Разрешение' }
                ].map((s) => (
                  <button key={s.id} onClick={() => setStage(s.id)} className={`text-left p-6 rounded-[2rem] border-2 transition-all duration-300 relative ${stage === s.id ? 'border-indigo-600 bg-white shadow-2xl shadow-indigo-100' : 'border-slate-100 bg-white hover:border-slate-300 shadow-sm'}`}>
            ...
            <section className={`p-8 rounded-[2.5rem] border-2 transition-all duration-500 ${isVerified ? 'bg-emerald-50 border-emerald-100' : 'bg-white border-amber-100 shadow-sm'}`}>
               <div className="flex gap-4 mb-6">
                 ...
                 <div className="space-y-1">
                   ...
                   <p className="text-[10px] font-bold text-slate-400 leading-relaxed uppercase tracking-tighter">ГИБДД, ФССП, Реестр залогов</p>
                 </div>
               </div>
               <label className={`flex items-center gap-4 p-5 bg-white border-2 rounded-2xl cursor-pointer transition-all ${isVerified ? 'border-emerald-500 shadow-lg shadow-emerald-100' : 'border-slate-100 hover:border-amber-200'}`}>
                 ...
                 <span className="text-xs font-black text-slate-700">Ограничения отсутствуют</span>
               </label>
            </section>
            ...
            <Button onClick={handleAnalyze} isLoading={isAnalyzing} className="w-full h-20 text-xl shadow-2xl shadow-indigo-200" disabled={files.length === 0 || !isVerified}>
              Начать юридический аудит
            </Button>
            {error && <div className="p-4 bg-rose-50 text-rose-600 text-[11px] font-black rounded-2xl border border-rose-100 text-center animate-pulse uppercase tracking-wider">{error}</div>}
          </div>
            ...
            <section className="space-y-6">
              ...
              <div className="relative group">
                ...
                <div className="bg-white border-2 border-dashed border-slate-200 rounded-[3rem] p-16 flex flex-col items-center justify-center text-center group-hover:bg-indigo-50/20 group-hover:border-indigo-400 transition-all duration-500">
                  <div className="w-16 h-16 bg-slate-50 rounded-3xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform text-slate-300 group-hover:bg-indigo-600 group-hover:text-white">
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                  </div>
                  <h5 className="text-xl font-black text-slate-900 tracking-tight">Перетащите файлы сюда</h5>
                  <p className="text-xs text-slate-400 font-bold mt-2 uppercase tracking-widest">PDF, JPEG, PNG (До 10 шт.)</p>
                </div>
              </div>
              ...
              {files.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {files.map((file, idx) => (
                    <div key={idx} className="flex items-center justify-between p-5 bg-white border border-slate-100 rounded-3xl shadow-sm hover:shadow-xl transition-all group overflow-hidden animate-fade-in">
                      ...
                      <button onClick={() => removeFile(idx)} className="p-2 text-slate-200 hover:text-rose-500 transition-colors">
        ...
        {result && (
          <section id="results" className="mt-20 space-y-12 animate-fade-in scroll-mt-32">
            <div className="bg-white rounded-[4rem] border border-slate-100 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] overflow-hidden">
              <div className="bg-slate-900 px-12 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-indigo-600 rounded-3xl flex items-center justify-center text-white shadow-2xl shadow-indigo-500/30">
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                  </div>
                  <div>
                    <h2 className="text-2xl font-black text-white tracking-tight">Заключение аудита</h2>
                    <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-[0.4em] mt-1">AI Legal Expert Analysis</p>
                  </div>
                </div>
                <div className="text-right">
                   <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">ОТЧЕТ СФОРМИРОВАН</div>
              ...
              <div className="p-12 md:p-20">
                <div className="prose prose-slate max-w-none">
                  <div className="whitespace-pre-wrap text-slate-700 leading-relaxed font-semibold text-lg custom-scrollbar max-h-[80vh] overflow-y-auto pr-8">
                    {result.text}
                  </div>
                </div>

                {stage === AppStage.FINAL && (
                  <div className="mt-16 p-10 bg-rose-50 rounded-[3rem] border border-rose-100 flex flex-col md:flex-row gap-8 items-start shadow-sm shadow-rose-100">
                    <div className="w-16 h-16 bg-rose-600 text-white rounded-2xl flex items-center justify-center flex-shrink-0 shadow-2xl shadow-rose-200">
                    ...
                    <div>
                      <h4 className="text-rose-900 font-black text-xs uppercase tracking-[0.3em] mb-3">Важное замечание</h4>
                      <p className="text-rose-800 text-lg leading-relaxed font-black italic">
                        "Открой заключение и фото! Сравни все досконально, не путай стороны расположения элементов. Любая неточность в описании сторон (лево/право) — причина отказа в ГАИ."
                      </p>
                    </div>
                  </div>
                )}
              </div>
              <div className="bg-slate-50 px-12 py-8 border-t border-slate-100 flex justify-between items-center">
                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400">AutoLegal Core v5.2</span>
                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-indigo-600 italic">Document Legally Scanned</span>
              </div>
            </div>
          </section>
        )}
      ...
      <footer className="py-20 border-t border-slate-100 bg-white/50 text-center space-y-4">
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.6em]">AutoLegal Expert • 2024</p>
        <p className="text-[10px] text-slate-300 font-bold max-w-xs mx-auto leading-relaxed">Профессиональный инструмент проверки документов ТС на базе ИИ.</p>
      </footer>
    </div>
  );
};
